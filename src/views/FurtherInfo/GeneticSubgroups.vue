  ...
  <!-- 频率类型切换按钮组 -->
    ...
    >
      ...
      >
        ...
        <div>
          <div class="chart-title">Allele Count in SubGroup Region</div>
          <div
...
const fetchMapData = async (index: number) => {
  ...
  const maxVal = Math.max(...data.map(d => d.value)) || 2000;
...
const downloadImages = async () => {
  const zip = new JSZip();
  const container = document.getElementById('export-area-region');
  if (!container) return;
